# Brief — Post S1-02 (Mercredi 2026-03-12)

## Stratégie

| Champ | Valeur |
|-------|--------|
| Pilier | La Cuisine (Process & Artisans) |
| Format | Carrousel 5 slides |
| Dimensions | 4:5 |
| Période | S1 — Le Reboot Premium |
| Position dans la grille | Post 2/12 du mur visuel |

## Objectif

Montrer le parcours "du boucher à l'assiette" en carrousel narratif. Crédibiliser le "fait maison" en rendant visible chaque étape de transformation. Ce post installe le storytelling artisanal et invite au swipe (signal d'engagement fort pour l'algorithme).

## Produit

**STRICT Bœuf** — Parcours de transformation, de la viande brute au burger fini.
Slug recette : `strict-boeuf`

> Ne PAS inclure de lien vers la photo référence ni la recette complète.
> L'input mapper résout automatiquement produit → photo + recette.

## Structure carrousel

| Slide | Contenu | Notes DA |
|-------|---------|----------|
| 1 (Cover) | Texte sur fond sombre : "De la Boucherie Labourde à ton assiette." | Fond #141210, typo Oswald en blanc, sous-titre Space Grotesk en #FABA43. **Image de grille** — doit être forte visuellement. |
| 2 | Viande crue sur planche, découpe artisanale | Plans serrés, gants noirs, bois de planche visible |
| 3 | Assemblage du burger, mains gantées, herbes | Le geste de précision — montrer le soin |
| 4 | Ajout verdure/finitions sur le burger | Verdure fraîche, contraste vert sur fond sombre |
| 5 | Burger fini, prêt à servir — hero shot | Le payoff du carrousel — le produit final dans toute sa gloire |

## Caption

```
3 artisans du coin dans chaque burger.

La viande ? Boucherie Labourde, à Perpignan.
Le pain ? Pains du Soleil, cuit du jour.
Le fromage ? Myfitcheese, protéiné et local.

On assemble tout ici, devant toi.
Cuisine ouverte, zéro huile, chaleur pulsée.

Swipe pour voir le parcours →

#strictfood #perpignan #cuisineouverte #faitmaison #artisanslocaux #boucherielabourde #painsdusoleil #myfitcheese #produitlocaux #perpignanfood #zerouile #chaleurpulsee #fastgood #restaurantperpignan
```

## Contraintes

- Slide 1 = cover graphique avec texte (template DA, pas de photo brute)
- Slides 2-4 = photos process, plans serrés, éclairage dramatique backstage
- Slide 5 = hero shot du produit fini (payoff visuel)
- La cover (slide 1) est l'image de grille — elle contraste avec S1-01 (hero shot doré) et crée du rythme
- DA : Dark Food Premium (fond Charbon, tons Cuivre Braisé)
- Food Porn Dial : 7/10

## Étape suivante

> Exécuter le skill `/social-media-art-director` avec ce brief.
> Output attendu dans `01-art-direction/direction.md`
